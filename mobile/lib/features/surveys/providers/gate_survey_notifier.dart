...
class GateSurveyState {
  ...
  // User tapped "Atla" in the current session; don't redirect to the gate again.
...
class GateSurveyNotifier extends Notifier<GateSurveyState>
  ...
  GateSurveyState build() {
    ...
    // Seed from the current value: ref.listen only fires on subsequent changes,
    // so if the underlying providers already resolved we'd otherwise stay stuck
    // in the loading state and never show the gate.
    return _fromAsync(ref.read(pendingGateSurveyProvider));
  }
