 ...
 *   - status    : 'draft'      (panelden yayınlanmadan önce gözden geçirin)
 *   - isGate    : true
 *   - isMandatory: false       (panelden zorunlu yapılabilir)
 *
 * Kullanım:
 *   # Emülatöre karşı (varsayılan):
...
async function main() {
  console.log(`\n🚀 PoM Genel Anket Seed — ${isProd ? 'PRODUCTION' : 'Emulator'}\n`);

  // Duplicate guard: check for existing active/draft gate survey with this title.
  ...
  if (!existing.empty) {
    console.log('⚠️  Zaten bir giriş anketi mevcut:');
    existing.forEach(d => {
  ...
  await surveyRef.set({
    ...
    emoji:         '📋',
    status:        'draft',       // Panelden inceleyip yayınlayın
    isGate:        true,
    isMandatory:   false,         // Panelden zorunlu yapılabilir
    questions:     QUESTIONS,
    minNThreshold: 5,
  ...
  console.log(`   durum   : draft  (yayınlamak için panel → Anketler → Yayınla)`);
  console.log(`   isGate  : true`);
  console.log(`   zorunlu : false  (panelden değiştirilebilir)\n`);
}
